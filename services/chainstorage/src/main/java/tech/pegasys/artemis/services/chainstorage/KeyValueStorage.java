...
public interface KeyValueStorage extends Closeable {
   ...
   */
  Optional<Bytes> get(Bytes key) throws StorageException;
  ...
  class Entry {
    ...
    private Entry(final Bytes key, final Bytes value) {
      this.key = key;
      this.value = value;
    }

    public static Entry create(final Bytes key, final Bytes value) {
  ...
  abstract class AbstractTransaction implements Transaction {
    ...
    @Override
    public final void put(final Bytes key, final Bytes value) {
      checkState(active, "Cannot invoke put() on a completed transaction.");
      doPut(key, value);
    }

    @Override
    public final void remove(final Bytes key) {
    ...
    protected abstract void doPut(Bytes key, Bytes value);

    protected abstract void doRemove(Bytes key);
