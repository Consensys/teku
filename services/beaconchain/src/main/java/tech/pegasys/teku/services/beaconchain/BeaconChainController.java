...
public class BeaconChainController extends Service implements TimeTickChannel {
  ...
  private void initPerformanceTracker() {
    ...
    if (config.isValidatorPerformanceTrackingEnabled()) {
      performanceTracker = new PerformanceTracker(recentChainData, STATUS_LOG);
      eventChannels.subscribe(SlotEventsChannel.class, performanceTracker);
    } else {
      LOG.info("validator-performance-tracking-enabled is false, not starting validator performance tracker.");
    }
  }
