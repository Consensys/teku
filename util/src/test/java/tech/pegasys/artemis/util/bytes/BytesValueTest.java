...
import static tech.pegasys.artemis.util.bytes.BytesValue.fromHexStringLenient;
...
public class BytesValueTest {
    ...
    /**
     * Checks that modifying a wrapped array modifies the value itself, but only if within the
     * wrapped slice.
     */
    @Test
    public void wrapSliceReflectsUpdates() {
        ...
        // Modify the bytes outside of the wrapped slice and check this doesn't affect the value
        // (that
    ...
    public void wrapBufferSliceReflectsUpdates() {
        ...
        // Modify within the wrapped slice. This should have modified the wrapped value but not its
        // copy
        ...
        assertArrayEquals(Arrays.copyOfRange(buffer.getBytes(), 2, 4), value.extractArray());
        assertFalse(
                Arrays.equals(Arrays.copyOfRange(buffer.getBytes(), 2, 4), copy.extractArray()));
    ...
    public void bytes() {
        ...
        assertArrayEquals(
                new byte[] {-1, 2, -3},
                BytesValue.of((byte) -1, (byte) 2, (byte) -3).extractArray());
    }
    ...
    public void integers() {
        ...
        assertArrayEquals(
                new byte[] {-1, 127, -128}, BytesValue.of(0xff, 0x7f, 0x80).extractArray());
    }
    ...
    public void hexStringLenientLeftPadding() {
        ...
        assertEquals(
                BytesValue.of(0x00, 0x00, 0x01, 0xff, 0x2a), fromHexStringLenient("0x1ff2a", 5));
        assertEquals(BytesValue.of(0x00, 0x01, 0xff, 0x2a), fromHexStringLenient("0x1fF2a", 4));
    ...
    public void getIntNotEnoughBytes() {
        thrown.expect(IllegalArgumentException.class);
        thrown.expectMessage(
                "Value of size 4 has not enough bytes to read a 4 bytes int from index 1");
        wrap(new byte[] {1, 2, 3, 4}).getInt(1);
    }
    ...
    public void arrayExtraction() {
        ...
        // But on top of the basic, extractArray() guarantees modifying the returned array is safe
        // from
    ...
    private void assertArrayExtraction(Function<BytesValue, byte[]> extractor) {
        ...
        byte[][] toTest =
                new byte[][] {
                    new byte[] {1}, new byte[] {1, 2, 3, 4, 5, 6}, new byte[] {-1, -1, 0, -1}
                };
        ...
        assertArrayEquals(
                new byte[] {}, extractor.apply(wrap(new byte[] {1, 2, 3, 4, 5}).slice(2, 0)));
    }
