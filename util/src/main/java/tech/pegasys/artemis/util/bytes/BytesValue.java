...
public interface BytesValue extends Comparable<BytesValue> {
     ...
     *
     * @param value The value to wrap.
     * @param offset The index (inclusive) in {@code value} of the first byte exposed by the
     *     returned value. In other words, you will have {@code wrap(value, o, l).get(0) ==
     *     value[o]}.
     ...
     *
     * <p>Note that the values are not copied, only wrapped, and thus any future update to {@code
     * v1} or {@code v2} will be reflected in the returned value. If copying the inputs is desired
    ...
    static BytesValue fromHexStringLenient(String str, int destinationSize) {
        checkArgument(
                destinationSize >= 0, "Invalid negative destination size %s", destinationSize);
     ...
     * is, the underlying value will be immutable) and 2) to not retain more bytes than exposed by
     * the value.
     *
     ...
     *     {@code this.size() &gt; (destination.size() - destinationOffset)}.
     */
     ...
     * return the backing array of this value if said value is array backed and doing so is
     * possible. As such, modifications to the returned array may or may not impact this value. As
     * such, this method should be used with care and hence the "unsafe" moniker.
     *
     * @return A byte array with the same content than this value, which may or may not be the
     *     direct backing of this value.
     */
