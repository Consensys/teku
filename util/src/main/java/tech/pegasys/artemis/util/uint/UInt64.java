...
public class UInt64 implements Comparable<UInt64> {
     ...
     * @return A new UInt64 instance representing the given unsigned input.
     * @throws NumberFormatException If the argument cannot be parsed as an unsigned integer. (i.e.
     *     <0 OR >2^64-1)
     */
     ...
     * so unsigned integers between 2^63-1 and 2^64-1 MUST be representated as a negative literal.
     *
     * <p>If this is not desired, please use {@link #plus(UInt64)}, i.e.
     * plus(UInt64.valueOf(String))
     *
     ...
     *
     * @param unsignedSubtrahend An unsigned long to subtract. Please see note above about support
     *     for large unsigned longs between 2^63-1 and 2^64-1.
     ...
     *
     * @param unsignedMultiplier An unsigned long to multiply. Please see note above about support
     *     for large unsigned longs between 2^63-1 and 2^64-1.
    ...
    /**
     * Divides the divisor passed in the argument by the specified object. The result is returned as
     * a new UInt64.
     *
     * <p><strong>NOTE: </strong> Java will not natively allow integer literals larger than 2^63-1,
     * so unsigned integers between 2^63-1 and 2^64-1 MUST be representated as a negative literal.
