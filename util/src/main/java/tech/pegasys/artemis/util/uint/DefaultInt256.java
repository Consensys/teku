...
class DefaultInt256 extends AbstractBytes32Backed implements Int256 {
    ...
    public int compareTo(Int256 other) {
        ...
        if (thisNeg) {
            // We're negative, if the other isn't it is bigger, otherwise both negative => compare
            // same as
            // unsigned.
            return otherNeg ? UInt256Bytes.compareUnsigned(bytes(), other.bytes()) : -1;
        }

        // We're positive, if the other isn't we are bigger, otherwise both are positive and we can
        // use
        // unsigned comparison.
