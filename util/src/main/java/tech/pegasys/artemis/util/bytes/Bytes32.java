...
public interface Bytes32 extends BytesValue {
     ...
     *     returned value. In other words, you will have {@code wrap(value, i).get(0) == value[i]}.
     ...
     * @param bytes The bytes to wrap.
     * @param offset The index (inclusive) in {@code value} of the first byte exposed by the
     *     returned value. In other words, you will have {@code wrap(value, i).get(0) ==
     *     value.get(i)}.
     * @return A {@link Bytes32} that exposes the bytes of {@code value} from {@code offset}
     *     (inclusive) to {@code offset + 32} (exclusive).
    ...
    static Bytes32 leftPad(BytesValue value) {
        checkArgument(
                value.size() <= SIZE,
                "Expected at most %s bytes but got only %s",
                SIZE,
                value.size());
