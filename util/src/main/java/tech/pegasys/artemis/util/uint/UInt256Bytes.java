...
public abstract class UInt256Bytes {
    ...
    public static void subtract(Bytes32 v1, Bytes32 v2, MutableBytes32 result) {
        ...
        for (int i = INT_SIZE - 1; i >= 0; i--) {
            diff =
                    (a.getInt(i * 4) & LONG_MASK)
                            - (b.getInt(i * 4) & LONG_MASK)
                            - ((int) -(diff >> 32));
    ...
    public static void multiplyModulo(
            Bytes32 v1, Bytes32 v2, Bytes32 modulo, MutableBytes32 result) {
        if (modulo.isZero()) {
            result.clear();
        } else {
    ...
    public static void signExtend(Bytes32 v1, Bytes32 v2, MutableBytes32 result) {
        // Any value >= 31 imply an index <= 0, so no work to do (note that 0 itself is a valid
        // index,
    ...
    public static Bytes32 of(BigInteger v) {
        ...
        checkArgument(
                toCopy.size() <= Bytes32.SIZE + 1, "Argument too big (%s bytes)", toCopy.size());

        if (toCopy.size() == Bytes32.SIZE + 1) {
            // Extra byte is due to sign bit. As the number has been checked to be positive, this
            // must
            // be 0 (or it mean the extra byte is truly due to the number being to big to represent
            // with
            // 32 bytes).
