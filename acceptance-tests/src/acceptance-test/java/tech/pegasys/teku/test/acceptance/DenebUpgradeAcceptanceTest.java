...
public class DenebUpgradeAcceptanceTest extends AcceptanceTestBase {
  ...
  void shouldUpgradeToDeneb() throws Exception {
    ...
    tekuNode.waitForMilestone(SpecMilestone.DENEB);
    tekuNode.waitForNewBlock();
  }

  private BesuNode createBesuNode(final int genesisTime) {
    final int cancunTime =
        genesisTime + 4 * 2; // 4 slots, 2 seconds each (swift) - activate Cancun on first slot
    final Map<String, String> genesisOverrides = Map.of("cancunTime", String.valueOf(cancunTime));
    ...
    return createBesuNode(
        ...
        config ->
            config
                .withMergeSupport()
                .withGenesisFile("besu/cancunGenesis.json")
                .withP2pEnabled(true)
