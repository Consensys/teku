...
public class Eip4844ExecutionClientHandlerTest extends ExecutionHandlerClientTest {
  ...
  void engineGetPayload_bellatrixFork() throws ExecutionException, InterruptedException {
    ...
    final ExecutionClientHandler handler =
        new Eip4844ExecutionClientHandler(bellatrixSpec, executionEngineClient);
  ...
  void engineGetPayload_capellaFork() throws ExecutionException, InterruptedException {
    ...
    final ExecutionClientHandler handler =
        new Eip4844ExecutionClientHandler(capellaSpec, executionEngineClient);
    final ExecutionPayloadContext context = randomContext();
  ...
  @Test
  void engineGetPayload_eip4844Fork() throws ExecutionException, InterruptedException {
    ...
    verify(executionEngineClient).getPayloadV3(context.getPayloadId());
    assertThat(future).isCompleted();
    assertThat(future.get().getExecutionPayload()).isInstanceOf(ExecutionPayloadEip4844.class);
  }
  ...
  @Test
  void engineNewPayload_eip4844Fork() {
    final ExecutionClientHandler handler = getHandler();
