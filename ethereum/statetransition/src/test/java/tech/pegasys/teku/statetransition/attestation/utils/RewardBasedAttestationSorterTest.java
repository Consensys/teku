...
public class RewardBasedAttestationSorterTest {
  ...
  @Test
  void sort_shouldNotRecomputeDifferentEpochAttestations() {
    // AttCurrent (Val 0): Timely Source. Initial Reward: 1000*14 = 14000
    // This will be chosen first.
    final PooledAttestationWithData attCurrent =
        mockPooledAttestationWithData(
            STATE_SLOT.minus(1), CURRENT_EPOCH, List.of(0), List.of(TIMELY_SOURCE_FLAG_INDEX));

    // AttPrevious (Val 1): Timely Target. Initial Reward: 1000*26 = 26000
    // Should be chosen first if AttCurrent wasn't present or had lower reward.
    // Its reward should not change when AttCurrent is processed.
    final PooledAttestationWithData attPrevious =
        mockPooledAttestationWithData(
            STATE_SLOT.minus(35), // Slot in previous epoch
            PREVIOUS_EPOCH,
            List.of(1),
            List.of(TIMELY_TARGET_FLAG_INDEX));

    // AttCurrent2 (Val 0): Timely Target. Initial Reward: 1000*26 = 26000
    // If AttCurrent is picked and sets TS for Val0, this attCurrent2 is for TT, so its reward
    // should not change based on TS.
    // However, it's in current epoch, so it WILL be recomputed.
    final PooledAttestationWithData attCurrent2 =
        mockPooledAttestationWithData(
            STATE_SLOT.minus(2), CURRENT_EPOCH, List.of(0), List.of(TIMELY_TARGET_FLAG_INDEX));

    final List<PooledAttestationWithData> attestations =
        List.of(attCurrent, attPrevious, attCurrent2);
    // Initial rewards:
    // AttPrevious: 26000
    // AttCurrent2: 26000
    // AttCurrent:  14000
    // Initial order: AttPrevious, AttCurrent2, AttCurrent (or AttCurrent2, AttPrevious)

    final List<RewardBasedAttestationSorter.PooledAttestationWithRewardInfo> sortedAttestations =
        sorter.sort(attestations, 3);

    // Step 1: AttPrevious (26000) or AttCurrent2 (26000) is chosen. Let's assume stable sort /
    // input order preference: AttPrevious
    // If AttPrevious is chosen:
    //   - Previous epoch participation for Val 1 updated.
    //   - AttCurrent and AttCurrent2 are NOT recomputed (different epoch from AttPrevious).
    // Queue: AttCurrent2 (26000), AttCurrent (14000)
    //
    // Step 2: AttCurrent2 (26000) is chosen.
    //   - Current epoch participation for Val 0 updated with TT_FLAG.
    //   - AttCurrent IS recomputed. Its reward (Val 0, wants TS) is unaffected by TT_FLAG on Val 0.
    // Still 14000.
    // Queue: AttCurrent (14000)
    //
    // Step 3: AttCurrent (14000) is chosen.
    // Expected order: AttPrevious, AttCurrent2, AttCurrent

    assertThat(sortedAttestations).hasSize(3);
    assertThat(
            sortedAttestations.stream()
                .map(RewardBasedAttestationSorter.PooledAttestationWithRewardInfo::getAttestation))
        .containsExactly(attPrevious, attCurrent2, attCurrent);

    assertThat(sortedAttestations.get(0).getRewardNumerator())
        .isEqualTo(BASE_REWARD_VALUE.times(TIMELY_TARGET_WEIGHT)); // AttPrevious
    assertThat(sortedAttestations.get(1).getRewardNumerator())
        .isEqualTo(BASE_REWARD_VALUE.times(TIMELY_TARGET_WEIGHT)); // AttCurrent2
    assertThat(sortedAttestations.get(2).getRewardNumerator())
        .isEqualTo(BASE_REWARD_VALUE.times(TIMELY_SOURCE_WEIGHT)); // AttCurrent
  }
