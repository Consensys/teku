...
public class SimpleSidecarRetrieverTest {
  ...
  @Test
  public void findMatchingPeers() {
    final TestPeer custodyPeer1 =
        new TestPeer(stubAsyncRunner, custodyNodeIds.next(), Duration.ofMillis(100));
    final TestPeer custodyPeer2 =
        new TestPeer(stubAsyncRunner, custodyNodeIds.next(), Duration.ofMillis(100));
    final TestPeer nonCustodyPeer =
        new TestPeer(stubAsyncRunner, nonCustodyNodeIds.next(), Duration.ofMillis(100));
    ...
    final BeaconBlock block = blockResolver.addBlock(10, 1);
    final DataColumnSlotAndIdentifier id0 = createId(block, columnIndex.intValue());
    final SimpleSidecarRetriever.RetrieveRequest retrieveRequest =
        new SimpleSidecarRetriever.RetrieveRequest(id0);
    testPeerManager.connectPeer(custodyPeer1);
    testPeerManager.connectPeer(custodyPeer2);
    testPeerManager.connectPeer(nonCustodyPeer);
    final SimpleSidecarRetriever.RequestTracker requestTracker =
        simpleSidecarRetriever.createFromCurrentPendingRequests();

    assertThat(simpleSidecarRetriever.findMatchingPeers(retrieveRequest, requestTracker).toList())
        .satisfies(
            result -> {
              final List<UInt256> nodeIds =
                  result.stream().map(SimpleSidecarRetriever.ConnectedPeer::getNodeId).toList();
              assertThat(nodeIds)
                  .containsExactlyInAnyOrder(custodyPeer1.getNodeId(), custodyPeer2.getNodeId());
            });
  }

  @Test
  public void findGoodMatchingPeers() {
    final TestPeer custodyPeer1 =
        new TestPeer(stubAsyncRunner, custodyNodeIds.next(), Duration.ofMillis(100));
    custodyPeer1.currentRequestLimit(10_000);
    final TestPeer custodyPeer2 =
        new TestPeer(stubAsyncRunner, custodyNodeIds.next(), Duration.ofMillis(100));
    final TestPeer nonCustodyPeer =
        new TestPeer(stubAsyncRunner, nonCustodyNodeIds.next(), Duration.ofMillis(100));
    ...
    assertThat(simpleSidecarRetriever.findGoodMatchingPeers(id0, requestTracker).toList())
        .satisfies(
            result -> {
              final List<UInt256> nodeIds =
                  result.stream().map(SimpleSidecarRetriever.ConnectedPeer::getNodeId).toList();
              // request limit above 8k goes first
              assertThat(nodeIds)
                  .containsExactly(custodyPeer1.getNodeId(), custodyPeer2.getNodeId());
            });
  }
}
