...
class MatchingDataAttestationGroupV2Test {
    ...
    public void onAttestationIncludedInBlock_multipleCallsShouldAggregate() {
        ...
        // onAttestationIncludedInBlock(attestation2) -> includedValidators gets bit 2. Attestation (2) is removed.
        ...
        // onBlock(att1=(1)): includedValidators=(1). (3) not removed. (1,2) not removed (2 not covered). size=2. numRemoved=0
        // onBlock(att2=(2)): includedValidators=(1,2). (3) not removed. (1,2) is removed. size=1. numRemoved for this call = 1
        // Total numRemoved = 1.
    ...
    @TestTemplate
    public void onAttestationIncludedInBlock_shouldRemoveAttestationsThatAreAggregatedIntoRemovedAttestation() {
    ...
    public void add_shouldAggregateAttestationsFromSameCommittee(final SpecContext specContext) {
        ...
        assertThat(
                group.streamForAggregationProduction(Optional.of(UInt64.ZERO), Long.MAX_VALUE))
    ...
    public void iterator_shouldAggregateAttestationsWithMoreValidatorsFirst() {
        ...
        verifyGroupContainsExactly(toPooledAttestationWithData(
                        PooledAttestation.fromValidatableAttestation(
                                ValidatableAttestation.from(
                                        spec,
    ...
    private PooledAttestationWithData toPooledAttestationWithDataWithSortedValidatorIndices(final PooledAttestationWithData attestation) {
    ...
    void verifyGroupContainsExactly(
        ...
        assertThat(group.streamForApiRequest(Optional.empty(),spec.getGenesisSpec().getMilestone().isGreaterThanOrEqualTo(ELECTRA))
    ...
    public void iterator_shouldOmitAttestationsThatAreAlreadyIncludedInTheAggregate() {
        ...
        // streamForBlockProduction will only show 'aggregate' (if not Electra, or if Electra and it's an aggregate)
        ...
        assertThat(group.streamForBlockProduction(Long.MAX_VALUE)).containsExactly(
                toPooledAttestationWithData(aggregate));
    }
    ...
    void onAttestationIncludedInBlock_shouldNotAddAttestationsAlreadySeenInBlocks() {
        ...
        assertThat(group.add(createPooledAttestation(1, 2, 3, 4, 5, 6), Optional.empty()))
                .isFalse();
    ...
    void onReorg_shouldAllowReadingAttestationsThatAreNoLongerRedundant() {
        ...
        assertThat(group.size()).isEqualTo(1);
        assertThat(group.streamForBlockProduction(Long.MAX_VALUE)).containsExactly(
                toPooledAttestationWithData(attestation));
    }
    ...
    void verifyStreamForAggregationProductionPhase0ContainsExactly(
            final PooledAttestationWithData... expectedAttestations) {
        assertThat(group.streamForAggregationProduction(Optional.empty(), Long.MAX_VALUE)
                .map(this::toPooledAttestationWithDataWithSortedValidatorIndices))
                .containsExactly(expectedAttestations);
    }
    ...
    void verifyStreamForApiRequest(final Optional<UInt64> committeeIndex, final boolean requiresCommitteeBits,
                                   final PooledAttestationWithData... expectedAttestations) {
        assertThat(group.streamForApiRequest(committeeIndex, requiresCommitteeBits)
                .map(this::toPooledAttestationWithDataWithSortedValidatorIndices))
                .containsExactly(expectedAttestations);
    }
    ...
    private PooledAttestation createPooledAttestation(
        ...
        return PooledAttestation.fromValidatableAttestation(validatableAttestation,
                validatorBitToValidatorIndex(validators));
    }

    private List<UInt64> validatorBitToValidatorIndex(final int... validatorBits) {
        return Arrays.stream(validatorBits)
                .sorted() // make sure we sort validators so that we can compare the list without converting it to a set
    ...
    private PooledAttestationWithData toPooledAttestationWithData(final Attestation attestation) {
        return new PooledAttestationWithData(
                ...
                PooledAttestation.fromValidatableAttestation(
                        ...
                        validatorBitToValidatorIndex(attestation.getAggregationBits().getAllSetBits().toIntArray())));
    }
}