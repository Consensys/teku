...
public class ForkChoiceUtil {
   ...
   * @param slot
   * @return
   * @see <a>https://github.com/ethereum/eth2.0-specs/blob/v0.8.1/specs/core/0_fork-choice.md#get_ancestor</a>
   */
   ...
   * @param store
   * @param root
   * @see <a>https://github.com/ethereum/eth2.0-specs/blob/v0.8.1/specs/core/0_fork-choice.md#get_latest_attesting_balance</a>
   */
   ...
   * @param store
   * @return
   * @see <a>https://github.com/ethereum/eth2.0-specs/blob/v0.8.1/specs/core/0_fork-choice.md#get_head</a>
   */
  public static Bytes32 get_head(Store store) {
    ...
    while (true) {
      ...
      head = children.stream()
                      .filter(
                              child ->
                                      get_latest_attesting_balance(store, child).compareTo(max) == 0)
  ...
  public static void on_block(Store store, BeaconBlock block) {
    ...
    checkArgument(store.getTime().compareTo(
            pre_state.getGenesis_time().plus(block.getSlot()
    ...
    checkArgument(get_ancestor(
            store,
            block.signing_root("signature"),
            store.getBlocks().get(store.getFinalized_checkpoint().getRoot()).getSlot()).equals(store.getFinalized_checkpoint().getRoot()),
            "on_block: Check block is a descendant of the finalized block");

    checkArgument(block.getSlot().compareTo(compute_start_slot_of_epoch(store.getFinalized_checkpoint().getEpoch())) > 0,
    ...
    if (state.getCurrent_justified_checkpoint().getEpoch().compareTo(store.getJustified_checkpoint().getEpoch()) > 0) {
      store.setJustified_checkpoint(state.getCurrent_justified_checkpoint());
    }

    // Update finalized checkpoint
    if (state.getFinalized_checkpoint().getEpoch().compareTo(store.getFinalized_checkpoint().getEpoch()) > 0) {
  ...
  public static void on_attestation(Store store, Attestation attestation) {
    ...
    checkArgument(store.getTime().compareTo(attestation_slot.plus(UnsignedLong.ONE).times(UnsignedLong.valueOf(SECONDS_PER_SLOT))) >= 0,
    ...
    checkArgument(is_valid_indexed_attestation(target_state, indexed_attestation),
    ...
    for (UnsignedLong i : all_indices) {
      if (!store.getLatest_messages().containsKey(i) || target.getEpoch().compareTo(store.getLatest_messages().get(i).getEpoch()) > 0) {
        store.getLatest_messages().put(i, new LatestMessage(target.getEpoch(), attestation.getData().getBeacon_block_root()));
      }
    }
  }
}
