...
public class AggregatingAttestationPoolBenchmark {
    ...
    private static final String STATE_PATH = "/Users/tbenr/Downloads/att pool test/BeaconStateDeneb_3630479_03664f196162fb81a4406c508674dd1ede09b883d37d0f3d0f076897f68741d2.ssz";
    private static final String POOL_DUMP_PATH = "/Users/tbenr/Downloads/att pool test/attestations_3630479.multi_ssz";
...
private AttestationForkChecker attestationForkChecker;
    ...
    public void init() throws Exception {
        ...
        this.pool = new AggregatingAttestationPool(SPEC, recentChainData, new NoOpMetricsSystem(), DEFAULT_MAXIMUM_ATTESTATION_COUNT);
        this.recentChainData = mock(RecentChainData.class);

        try (final FileInputStream fileInputStream = new FileInputStream(STATE_PATH)) {
            this.state = SPEC.getGenesisSpec().getSchemaDefinitions().getBeaconStateSchema().sszDeserialize(Bytes.wrap(fileInputStream.readAllBytes()));
        }
        ...
        try (final Stream<String> attestationLinesStream = Files.lines(Paths.get(POOL_DUMP_PATH))) {
            attestationLinesStream.map(line -> attestationSchema.sszDeserialize(Bytes.fromHexString(line)))
    ...
    public static void main(String[] args) throws Exception {
        AggregatingAttestationPoolBenchmark benchmark = new AggregatingAttestationPoolBenchmark();
            benchmark.init();
            benchmark.getAttestationsForBlock(new Blackhole("Today's password is swordfish. I understand instantiating Blackholes directly is dangerous."));
    }
}
