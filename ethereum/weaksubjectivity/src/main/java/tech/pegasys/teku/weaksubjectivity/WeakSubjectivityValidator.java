...
public class WeakSubjectivityValidator {
  ...
  public static WeakSubjectivityValidator lenient(final WeakSubjectivityConfig config, final Optional<BeaconStateUtil> beaconStateUtil) {
    final WeakSubjectivityCalculator calculator = WeakSubjectivityCalculator.create(config);
    return new WeakSubjectivityValidator(
  ...
  public void validateLatestFinalizedCheckpoint(
    ...
    final UInt64 currentEpoch = maybeBeaconStateUtil.map(stateUtil -> stateUtil.computeEpochAtSlot(currentSlot)).orElse(compute_epoch_at_slot(currentSlot));
  ...
  public boolean isBlockValid(
    ...
    final UInt64 blockEpoch = maybeBeaconStateUtil.map(stateUtil -> stateUtil.computeEpochAtSlot(block.getSlot())).orElse(compute_epoch_at_slot(block.getSlot()));
    boolean blockAtEpochBoundary = maybeBeaconStateUtil.map(stateUtil -> stateUtil.computeStartSlotAtEpoch(blockEpoch)).orElse(compute_start_slot_at_epoch(blockEpoch)).equals(block.getSlot());
  ...
  Optional<UInt64> getSuppressWSPeriodChecksUntilEpoch(final UInt64 currentSlot) {
    if (suppressWSPeriodErrorsUntilEpoch.isEmpty()
      ...
      final UInt64 startupEpoch = maybeBeaconStateUtil.map(stateUtil -> stateUtil.computeEpochAtSlot(currentSlot)).orElse(compute_epoch_at_slot(currentSlot));
