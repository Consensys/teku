...
public interface RLPOutput {
     ...
     *
     * @throws IllegalStateException if no list has been previously started with {@link
     *     #startList()} (or any started had already be ended).
     */
    void endList();
    ...
    default void writeBigIntegerScalar(BigInteger v) {
        ...
        // bit of sign (a zero here since it's positive). What that mean is that if the first 1 of
        // the
        // resulting number is exactly on a byte boundary, then the sign bit constraint will make
        // the
        // value include one extra byte, which will be zero. In other words, they can be one zero
        // bytes
     ...
     * @throws IllegalArgumentException if {@code b} does not fit an unsigned byte, that is if
     *     either {@code b < 0} or {@code b > 0xFF}.
     */
     ...
     * @throws IllegalArgumentException if {@code i} does not fit an unsigned 4-bytes int, that is
     *     if either {@code i < 0} or {@code i > 0xFFFFFFFFL}.
     */
     ...
     * fully formed Java object) and then re-encoding that result to this output. It is however a
     * lot more efficient in that it saves most of that decoding/re-encoding work. Please note
     * however that this method <b>does</b> validate that the input is a valid RLP encoding. If you
     * can guaranteed that the input is valid and do not want this validation step, please have a
     * look at {@link #writeRLPUnsafe(BytesValue)}.
     *
