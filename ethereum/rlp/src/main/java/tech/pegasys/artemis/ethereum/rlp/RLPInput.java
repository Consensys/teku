...
public interface RLPInput {
     ...
     *
     * <p>This method is equivalent to calling {@link #leaveList(boolean)} with value
     * <tt>false</tt>.
     *
     * <p>Note that this method technically doesn't consume any input but must be called after
     * having read the last element of a list. This allow to ensure the structure of the input is
     ...
     * indeed the one expected.
     *
     * @param ignoreRest Whether to ignore any remaining elements in the list. If elements remain
     *     and this parameter is <tt>false</tt>, an exception will be thrown.
     ...
     * @throws RLPException if the next item to read is a list, the input is at the end of its
     *     current list (and {@link #leaveList()} hasn't been called) or if the next item is either
     *     too big to fit a long or has leading zeros.
     */
    long readLongScalar();
     ...
     * @throws RLPException if the next item to read is a list, the input is at the end of its
     *     current list (and {@link #leaveList()} hasn't been called) or if the next item has
     *     leading zeros.
     */
     ...
     * @throws RLPException if the next item to read is a list, the input is at the end of its
     *     current list (and {@link #leaveList()} hasn't been called) or if the next item is either
     *     too big to fit a {@link UInt256} or has leading zeros.
     */
    UInt256 readUInt256Scalar();

    /**
     * Reads a scalar of maximum 32 bytes from the input and pass it to the provided value to create
     * a corresponding {@link UInt256Value}.
     *
     * <p>Note that for convenience, any exception thrown by the provided method will be wrapped in
     * a {@link RLPException} (it is considered as a "decoding error").
     *
     ...
     * @return The value created from applying {@code bytesWrapper} to the scalar read from that
     *     input (eventually padded to fit 32 bytes).
     * @throws RLPException if the next item to read is a list, the input is at the end of its
     *     current list (and {@link #leaveList()} hasn't been called) or if the next item is either
     *     too big to fit a {@link UInt256} or has leading zeros.
     */
     ...
     * @throws RLPException if the next item to read is a list, the input is at the end of its
     *     current list (and {@link #leaveList()} hasn't been called) or if the next item is not a
     *     single byte long.
     */
    byte readByte();
     ...
     * @throws RLPException if the next item to read is a list, the input is at the end of its
     *     current list (and {@link #leaveList()} hasn't been called) or if the next item is not
     *     2-bytes.
     */
    short readShort();
     ...
     * @throws RLPException if the next item to read is a list, the input is at the end of its
     *     current list (and {@link #leaveList()} hasn't been called) or if the next item is neither
     *     4 nor 16 bytes.
     */
     ...
     * @return The next item read of this input.
     * @throws RLPException if the next item to read is a list, the input is at the end of its
     *     current list (and {@link #leaveList()} hasn't been called) or the next element is not
     *     exactly 32 bytes.
     */
    Bytes32 readBytes32();
     ...
     *
     * <p>Note that the only benefit of this method over calling the mapper function on the result
     * of {@link #readBytesValue()} is that any error thrown by the mapper will be wrapped by a
     * {@link RLPException}, which can make error handling more convenient (having a particular
     * decoded value not match what is expected is not fundamentally different from trying to read
     * an unsigned short from an item with strictly more or less than 2 bytes).
     *
     ...
     * @throws RLPException if the next item to read is a list, the input is at the end of its
     *     current list (and {@link #leaveList()} hasn't been called) or {@code mapper} throws an
        ...
        for (int i = 0; i < size; i++) {
            try {
                ...
                throw new RLPException(
                        String.format(
                                "Error applying element decoding function on "
                                        + "element %d of the list",
                                i),
                        e);
            }
        }
