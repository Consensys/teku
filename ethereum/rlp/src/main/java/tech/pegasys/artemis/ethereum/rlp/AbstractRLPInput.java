...
abstract class AbstractRLPInput implements RLPInput {
    ...
    // Information regarding opened list. The depth is how many list deep we are, and
    // endOfListOffset
    ...
    protected void init(long inputSize, boolean shouldFitInputSizeExactly) {
        ...
        currentItem = 0;
        // Initially set the size to the input as prepareCurrentTime() needs it. Once we've prepare
        // the
        ...
        // No matter what, if the first item advertise a payload ending after the end of the input,
        // that
        // input is corrupted.
        if (size > inputSize) {
            // Our error message include a snippet of the input and that code assume size is not set
            // outside of the input, and that's exactly the case we're testing, so resetting the
            // size
    ...
    protected void setTo(long item) {
        currentItem = item;
        if (currentItem >= size) {
            // Setting somewhat safe values so that multiple calls to setTo(nextItem()) don't do
            // anything
    ...
    private void validateCurrentItem() {
        if (currentKind == Kind.SHORT_ELEMENT) {
            // Validate that a single byte SHORT_ELEMENT payload is not <= 0x7F. If it is, is should
            // have
    ...
    private int readLongSize(long item, int sizeLength) {
        // We will read sizeLength bytes from item + 1. There must be enough bytes for this or the
        // input
        // is corrupted.
        ...
        if (res < 56) {
            throwMalformed(
                    "Malformed RLP item: written as a long item, but size %d < 56 bytes", res);
        }
    ...
    public boolean isDone() {
        // The input is done if we're out of input, but also if we've called leaveList() an
        // appropriate
        // amount of times.
