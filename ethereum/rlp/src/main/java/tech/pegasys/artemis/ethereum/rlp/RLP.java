...
import static tech.pegasys.artemis.ethereum.rlp.RLPDecodingHelpers.extractSize;
import static tech.pegasys.artemis.ethereum.rlp.RLPEncodingHelpers.elementSize;
import static tech.pegasys.artemis.ethereum.rlp.RLPEncodingHelpers.isSingleRLPByte;
...
public abstract class RLP {
     ...
     * @throws MalformedRLPInputException if {@code encoded} doesn't contain a single RLP encoded
     *     item (item that can be a list itself). Note that more deeply nested
     *     corruption/malformation of the input will not be detected by this method call, but will
     *     be later when the input is read.
     */
     ...
     * @return The output of decoding {@code value}. It will be either directly a {@link
     *     BytesValue}, or a list whose elements are either {@link BytesValue}, or similarly
     *     composed sub-lists.
     ...
     *
     * <p>This method is mostly intended for testing as it is often more convenient <b>and</b>
     * efficient to use a {@link RLPOutput} (through {@link #encode(Consumer)} for instance)
     * instead.
     *
    ...
    /**
     * Creates a {@link RLPOutput}, pass it to the provided consumer for writing, and then return
     * the RLP encoded result of that writing.
     *
