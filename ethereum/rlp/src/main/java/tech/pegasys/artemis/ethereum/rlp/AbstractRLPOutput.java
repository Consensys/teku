...
import static tech.pegasys.artemis.ethereum.rlp.RLPEncodingHelpers.listSize;
...
abstract class AbstractRLPOutput implements RLPOutput {
     ...
     * @throws IllegalStateException if some opened list haven't been closed (the output is not
     *     valid as is).
     */
    public int encodedSize() {
        checkState(
                stackSize == 1, "A list has been entered (startList()) but not left (endList())");
        return payloadSizes[0];
    }

    protected void writeEncoded(MutableBytesValue res) {
        // Special case where we encode only a single non-list item (note that listsCount is
        // initially
