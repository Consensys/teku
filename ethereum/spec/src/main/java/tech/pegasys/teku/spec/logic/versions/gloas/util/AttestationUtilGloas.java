...
public class AttestationUtilGloas extends AttestationUtilElectra {
  ...
  public AttestationValidationResult validateIndexValue(final AttestationData attestationData) {
    ...
    if (attestationData.getPayloadStatus().isEmpty()
        || !attestationData.getPayloadStatusRequired().isLessThan(2)) {
      return AttestationValidationResult.invalid(
          () ->
              String.format(
                  "Attestation data index must be 0 or 1 for Gloas, but was %s.",
                  attestationData.getPayloadStatus().orElse(null)));
    }
        ...
        .map(
            blockSlot -> {
              if (blockSlot.equals(attestationData.getSlot())
                  && !attestationData.getPayloadStatusRequired().isZero()) {
                return AttestationValidationResult.invalid(
                    () ->
                        String.format(
                            "Payload status must be 0, but was %s.",
                            attestationData.getPayloadStatusRequired()));
              } else {
                return AttestationValidationResult.VALID;
              }
            })
