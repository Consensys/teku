...
public class IndexedAttestationSchemaProvider
  ...
  public IndexedAttestationSchemaProvider() {
    super(SchemaTypes.INDEXED_ATTESTATION_SCHEMA);
    addMilestoneMapping(PHASE0, DENEB);
  }

  @Override
  protected IndexedAttestationSchema<IndexedAttestation> createSchema(
      final SchemaRegistry registry,
      final SpecMilestone effectiveMilestone,
      final SpecConfig specConfig) {
    return switch (effectiveMilestone) {
      case PHASE0 ->
          new IndexedAttestationPhase0Schema(specConfig.getMaxValidatorsPerCommittee())
              .castTypeToIndexedAttestationSchema();
      case ELECTRA ->
          new IndexedAttestationElectraSchema(
