...
### Breaking Changes
 ...
 - Removed the `--deposit-snapshot-enabled` and `--Xdeposit-snapshot` CLI options along with the bundled deposit tree snapshots. Both were no-ops after the removal of the Eth1 deposit-log fetching, since nothing loads a deposit tree snapshot any more.
