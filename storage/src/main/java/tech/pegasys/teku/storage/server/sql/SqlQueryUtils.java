...
public class SqlQueryUtils {
  ...
  public SqlQueryUtils(final JdbcOperations jdbc) {
    this.jdbc = jdbc;
  }
  ...
  protected <K, V> Map<K, V> loadMap(
      ...
      final Object... params) {
    return loadMap(new HashMap<>(), sql, keyExtractor, valueExtractor, params);
  }

  protected <M extends Map<K, V>, K, V> M loadMap(
      final M result,
      final String sql,
      final SqlFunction<K> keyExtractor,
      final SqlFunction<V> valueExtractor,
  ...
  protected Bytes32 getBytes32(final ResultSet rs, final String field) throws SQLException {
    final byte[] bytes = rs.getBytes(field);
    return bytes != null ? Bytes32.wrap(bytes) : null;
  }
