...
class ThrottlingSyncSourceTest {
  ...
  void shouldRequestBlocksImmediatelyIfRateLimitNotExceeded() {
    ...
    ignoreFuture(source.requestBlocksByRange(UInt64.ZERO, count, listener));
    ignoreFuture(source.requestBlocksByRange(UInt64.valueOf(100), count, listener));

    // Both requests happen immediately
    ignoreFuture(verify(delegate).requestBlocksByRange(UInt64.ZERO, count, listener));
    ignoreFuture(verify(delegate).requestBlocksByRange(UInt64.valueOf(100), count, listener));
  }
