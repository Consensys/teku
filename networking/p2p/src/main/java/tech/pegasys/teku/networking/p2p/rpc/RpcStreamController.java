...
public interface RpcStreamController<TOutgoingHandler extends RpcRequestHandler> {
  ...
  default TOutgoingHandler getRequiredOutgoingRequestHandler() {
    return getOutgoingRequestHandler()
        .orElseThrow(
            () -> new IllegalStateException("Expected outgoing request handler to be set"));
  }
}
