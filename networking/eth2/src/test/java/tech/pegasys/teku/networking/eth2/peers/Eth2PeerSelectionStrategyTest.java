...
class Eth2PeerSelectionStrategyTest {
  ...
  void selectPeersToConnect_shouldConnectToRandomPeers() {
    ...
    assertThat(peerPools.getPool(PEER3.getId())).isEqualTo(PeerConnectionType.RANDOMLY_SELECTED);
    assertThat(peerPools.getPool(PEER1.getId())).isEqualTo(PeerConnectionType.RANDOMLY_SELECTED);
  }
  ...
  void selectPeersToConnect_shouldExcludeRandomlySelectedPeersWhenSelectingBasedOnScore() {
    ...
    assertThat(peerPools.getPool(PEER1.getId())).isEqualTo(PeerConnectionType.RANDOMLY_SELECTED);
    assertThat(peerPools.getPool(PEER2.getId())).isEqualTo(PeerConnectionType.SCORE_BASED);
    assertThat(peerPools.getPool(PEER3.getId())).isEqualTo(PeerConnectionType.RANDOMLY_SELECTED);
  }
