...
public class Eth2GossipTopicFilter implements GossipTopicFilter {
  ...
  private Set<String> computeRelevantTopics(
    ...
    final Set<String> topics = getAllTopics(gossipEncoding, forkDigest, spec.getNetworkingConfig());
    spec.getForkSchedule().getForks().stream()
        ...
        .forEach(
            futureFork -> {
              ...
              topics.addAll(
                  getAllTopics(gossipEncoding, futureForkDigest, spec.getNetworkingConfig()));
            });
    return topics;
  }
}
