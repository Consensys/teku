    ...
    return recentChainData
        ...
        .thenApply(
            maybeState -> {
              ...
              // The current finalized_checkpoint is an ancestor of the block defined by aggregate.data.beacon_block_root
              if (!forkChoiceUtilWrapper
                      .get_ancestor(
                              recentChainData.getForkChoiceStrategy().orElseThrow(),
                              data.getBeacon_block_root(),
                              compute_start_slot_at_epoch(recentChainData.getFinalizedEpoch()))
                      .map(ancestorOfLMDVote -> ancestorOfLMDVote.equals(recentChainData.getFinalizedCheckpoint().orElseThrow().getRoot()))
                      .orElse(false)) {
                return REJECT;
              }
