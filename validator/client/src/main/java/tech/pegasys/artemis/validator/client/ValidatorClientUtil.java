  ...
  public static Optional<Triple<List<Integer>, UnsignedLong, UnsignedLong>>
    ...
    for (UnsignedLong slot = start_slot;
      ...
      int slot_start_shard =
          (get_start_shard(state, epoch).intValue() + offset) % MAX_COMMITTEES_PER_SLOT;

      for (int i = 0; i < committees_per_slot; i++) {
        UnsignedLong shard = UnsignedLong.valueOf((slot_start_shard + i) % MAX_COMMITTEES_PER_SLOT);
        List<Integer> committee = get_beacon_committee(state, epoch, shard);
