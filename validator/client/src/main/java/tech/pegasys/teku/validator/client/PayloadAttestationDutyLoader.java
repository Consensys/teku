...
import tech.pegasys.teku.validator.client.duties.attestations.PayloadAttestationDuty;
import tech.pegasys.teku.validator.client.loader.OwnedValidators;

public class PayloadAttestationDutyLoader
    extends AbstractDutyLoader<PayloadAttesterDuties, SlotBasedScheduledDuties<?, ?>> {

  private final ValidatorApiChannel validatorApiChannel;
  private final Function<Bytes32, SlotBasedScheduledDuties<PayloadAttestationDuty, Duty>>
      scheduledDutiesFactory;

  public PayloadAttestationDutyLoader(
      final ValidatorApiChannel validatorApiChannel,
      final Function<Bytes32, SlotBasedScheduledDuties<PayloadAttestationDuty, Duty>>
          scheduledDutiesFactory,
  ...
  @Override
  protected SafeFuture<Optional<PayloadAttesterDuties>> requestDuties(
      final UInt64 epoch, final IntCollection validatorIndices) {
    if (validatorIndices.isEmpty()) {
      return SafeFuture.completedFuture(Optional.empty());
    }
    return validatorApiChannel.getPayloadAttestationDuties(epoch, validatorIndices);
  }

  @Override
  protected SafeFuture<SlotBasedScheduledDuties<?, ?>> scheduleAllDuties(
      final UInt64 epoch, final PayloadAttesterDuties duties) {
    final SlotBasedScheduledDuties<PayloadAttestationDuty, Duty> scheduledDuties =
  ...
  private void scheduleDuty(
      final SlotBasedScheduledDuties<PayloadAttestationDuty, Duty> scheduledDuties,
      final PayloadAttesterDuty duty) {
    validators
