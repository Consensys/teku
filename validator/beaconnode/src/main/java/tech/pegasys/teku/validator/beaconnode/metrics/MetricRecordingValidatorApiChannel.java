...
public abstract class MetricRecordingValidatorApiChannel implements ValidatorApiChannel {
  ...
  private <T> SafeFuture<T> countDataRequest(
    ...
    return request
        .catchAndRethrow(__ -> recordError(methodLabel))
        .thenPeek(__ -> recordSuccess(methodLabel));
  }

  private <T> SafeFuture<Optional<T>> countOptionalDataRequest(
      final SafeFuture<Optional<T>> request, final String methodLabel) {
    return request
        .catchAndRethrow(__ -> recordError(methodLabel))
        .thenPeek(
            result ->
                result.ifPresentOrElse(
                    __ -> recordSuccess(methodLabel), () -> recordDataUnavailable(methodLabel)));
  }

  private <T> SafeFuture<List<T>> countSendRequest(
      final SafeFuture<List<T>> request, final String methodLabel) {
    return request
        .catchAndRethrow(__ -> recordError(methodLabel))
        .thenPeek(
            result -> {
              if (result.isEmpty()) {
                recordSuccess(methodLabel);
              } else {
                recordError(methodLabel);
              }
            });
  }

  private void recordSuccess(final String methodLabel) {
    recordRequest(methodLabel, RequestOutcome.SUCCESS);
  }

  private void recordError(final String methodLabel) {
    recordRequest(methodLabel, RequestOutcome.ERROR);
  }

  private void recordDataUnavailable(final String methodLabel) {
