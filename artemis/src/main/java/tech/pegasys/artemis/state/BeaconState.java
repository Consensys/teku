...
public class BeaconState {
    ...
    private boolean validate_proof_of_possession(
        ...
        return bls_verify(
                UInt384.valueOf(pubkey),
                hash_tree_root(proof_of_possession_data),
                signature,
                domain);
    }
    ...
    public int process_deposit(
        ...
        assert validate_proof_of_possession(
                state, pubkey, proof_of_possession, withdrawal_credentials, randao_commitment);
        ...
        if (indexOfPubkey(validator_pubkeys, pubkey) == -1) {
            ...
            index =
                    min_empty_validator_index(
                            validators_copy, validator_balances, toIntExact(slot));
    ...
    public void exit_validator(BeaconState state, int index, int new_status) {
        ...
        if (new_status == EXITED_WITH_PENALTY) {
            ...
            latest_penalized_exit_balances.set(
                    lpeb_index,
                    latest_penalized_exit_balances.get(lpeb_index)
    ...
    public static ArrayList<ShardCommittee> get_shard_committees_at_slot(
        ...
        int earliest_slot_in_array =
                toIntExact(state.getSlot())
                        - (toIntExact(state.getSlot()) % EPOCH_LENGTH)
                        - EPOCH_LENGTH;
    ...
    /**
     * Returns the participant indices at for the ``attestation_data`` and
    ...
    public static ArrayList<ArrayList<ShardCommittee>> get_shuffling(
        ...
        for (int slot_position = 0; slot_position < validators_per_slot.size(); slot_position++) {
            ...
            long shard_id_start =
                    (long) crosslinking_start_shard + slot_position * committees_per_slot;
    ...
    /**
     * Assumes ``attestation_data_1`` is distinct from ``attestation_data_2``. Returns True if the
     * provided ``AttestationData`` are slashable due to a 'surround vote'. Note: parameter order
     * matters as this function only checks that ``attestation_data_1`` surrounds
     * ``attestation_data_2``.
     *
     * @param attestation_data_1
     * @param attestation_data_2
    ...
    private Hash get_new_validator_registry_delta_chain_tip(
            Hash current_validator_registry_delta_chain_tip,
            int validator_index,
            int pubkey,
            int flag) {
        return Hash.hash(
    ...
    static class BeaconStateHelperFunctions {
        ...
        /**
         * TODO: implement merkle_root helper function for updateBatched_block_roots() definition
    ...
    public void setValidator_registry_latest_change_slot(
            long validator_registry_latest_change_slot) {
        this.validator_registry_latest_change_slot = validator_registry_latest_change_slot;
    }
    ...
    public void setLatest_penalized_exit_balances(
            ArrayList<Double> latest_penalized_exit_balances) {
        this.latest_penalized_exit_balances = latest_penalized_exit_balances;
    }
}
