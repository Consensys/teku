...
/**
 * High-performance cache implementation using Caffeine's TinyLFU eviction policy. Provides
 * excellent concurrency and better hit rates than traditional LRU by considering both frequency and
 * recency of access. Best for high-contention bounded caches on hot paths.
 *
...
public final class CaffeineCache<K, V> implements Cache<K, V> {
  ...
  @Override
  public int size() {
    // Force pending maintenance operations to complete before returning size to ensure accuracy
    cache.cleanUp();
