...
public class SafeFuture<T> extends CompletableFuture<T> {
  ...
  /**
   * Repeatedly run the loop body futures until the while condition returns false
   *
   * @param loopBody A supplier for generating futures to be run in succession
   * @param whileCondition A supplier determining when to terminate the async loop
   * @param <T> The type of future returned by the loop body supplier
   * @return The last value generated by {@code loopBody}
   */
  public static <T> SafeFuture<T> asyncDoWhile(
      ExceptionThrowingFutureSupplier<T> loopBody, Supplier<Boolean> whileCondition) {
    return SafeFuture.of(loopBody)
        .thenCompose(
            res -> {
              if (!whileCondition.get()) {
                return SafeFuture.completedFuture(res);
              }
              return asyncDoWhile(loopBody, whileCondition);
            });
  }
