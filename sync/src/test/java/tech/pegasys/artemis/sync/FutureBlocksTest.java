...
public class FutureBlocksTest {
  ...
  @Test
  public void add() {
    final BeaconBlock block = DataStructureUtil.randomBeaconBlock(5, 1);
  ...
  public void prune_nothingToPrune() {
    ...
    final List<BeaconBlock> pruned = futureBlocks.prune(priorSlot);
  ...
  public void prune_blockAtSlot() {
    ...
    final List<BeaconBlock> pruned = futureBlocks.prune(block.getSlot());
  ...
  public void prune_blockPriorToSlot() {
    ...
    final List<BeaconBlock> pruned = futureBlocks.prune(block.getSlot().plus(UnsignedLong.ONE));
    assertThat(pruned).containsExactly(block);
    assertThat(futureBlocks.size()).isEqualTo(0);
  }
}
