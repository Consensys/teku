...
public class GetStateValidatorBalances extends AbstractHandler implements Handler {
  ...
  public void handle(@NotNull final Context ctx) throws Exception {
    try {
      ...
      final Function<Bytes32, SafeFuture<Optional<List<ValidatorBalanceResponse>>>> rootHandler = (root) ->
              chainDataProvider.getValidatorsBalancesByStateRoot(root, validatorIndices);
      final Function<UInt64, SafeFuture<Optional<List<ValidatorBalanceResponse>>>> slotHandler = (slot) ->
              chainDataProvider.getValidatorsBalancesBySlot(slot, validatorIndices);
