...
import static tech.pegasys.teku.ethereum.json.types.validator.InclusionListDuties.INCLUSION_LIST_DUTIES_TYPE_DEFINITION;
...
public class PostInclusionListDuties extends RestApiEndpoint {
  public static final String ROUTE = "/eth/v1/validator/duties/inclusion_list/{epoch}";
  private final ValidatorDataProvider validatorDataProvider;
  private final SyncDataProvider syncDataProvider;
  private final Spec spec;

  public PostInclusionListDuties(final DataProvider dataProvider, final Spec spec) {
    this(dataProvider.getSyncDataProvider(), spec, dataProvider.getValidatorDataProvider());
  }

  PostInclusionListDuties(
      final SyncDataProvider syncDataProvider,
      final Spec spec,
      final ValidatorDataProvider validatorDataProvider) {
    super(
        EndpointMetadata.post(ROUTE)
            .operationId("getInclusionListDuties")
            .summary("Get inclusion list duties")
            .description(
                "Requests the beacon node to provide a set of inclusion list duties, which "
            ...
            .response(SC_OK, "Success response", INCLUSION_LIST_DUTIES_TYPE_DEFINITION)
            .response(
  ...
  public void handleRequest(final RestApiRequest request) throws JsonProcessingException {
    ...
    if (spec.atEpoch(epoch).getMilestone().isLessThan(SpecMilestone.HEZE)) {
      request.respondError(
          SC_BAD_REQUEST, "Cannot request inclusion list duties for epochs prior to heze fork.");
      return;
    }
    ...
    final SafeFuture<Optional<InclusionListDuties>> future =
