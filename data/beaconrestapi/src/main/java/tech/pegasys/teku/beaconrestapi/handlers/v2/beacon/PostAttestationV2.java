...
import static tech.pegasys.teku.beaconrestapi.handlers.v1.beacon.MilestoneDependentTypesUtil.slotBasedSelectorForList;
...
public class PostAttestationV2 extends RestApiEndpoint {
  ...
  private static EndpointMetadata createMetadata(
    ...
    final SerializableOneOfTypeDefinition<List<Attestation>> attestationsListSchema =
        DeserializableTypeDefinition.listOf(attestationSchemaDefinition);

    final OneOfJsonRequestContentTypeDefinition.BodyTypeSelector<List<Attestation>>
        attestationBodySelector =
            context ->
                slotBasedSelectorForList(
                    context.getBody(),
                    schemaDefinitionCache,
                    SchemaDefinitions::getAttestationSchema);
    ...
    return EndpointMetadata.post(ROUTE)
        ...
        .requestBodyTypeForList(attestationsListSchema, attestationBodySelector)
        .headerRequired(
