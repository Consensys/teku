...
public class BeaconBlockBodyEip7594 extends BeaconBlockBodyAltair {

  @JsonProperty("execution_payload")
  public final ExecutionPayloadEip7594 executionPayload;
  ...
  public BeaconBlockBodyEip7594(
      ...
      @JsonProperty("execution_payload") final ExecutionPayloadEip7594 executionPayload,
      @JsonProperty("bls_to_execution_changes")
    ...
    super(
        ...
        voluntaryExits,
        syncAggregate);
    checkNotNull(executionPayload, "Execution Payload is required for EIP7594 blocks");
    this.executionPayload = executionPayload;
    checkNotNull(blsToExecutionChanges, "BlsToExecutionChanges is required for EIP7594 blocks");
    this.blsToExecutionChanges = blsToExecutionChanges;
    checkNotNull(blobKZGCommitments, "blobKZGCommitments is required for EIP7594 blocks");
    this.blobKZGCommitments = blobKZGCommitments;
  }

  public BeaconBlockBodyEip7594(
      tech.pegasys.teku.spec.datastructures.blocks.blockbody.versions.eip7594.BeaconBlockBodyEip7594
          message) {
    super(message);
    checkNotNull(message.getExecutionPayload(), "Execution Payload is required for EIP7594 blocks");
    this.executionPayload = new ExecutionPayloadEip7594(message.getExecutionPayload());
    checkNotNull(
        message.getBlsToExecutionChanges(),
        "BlsToExecutionChanges are required for EIP7594 blocks");
    this.blsToExecutionChanges =
        message.getBlsToExecutionChanges().stream().map(SignedBlsToExecutionChange::new).toList();
    checkNotNull(
        message.getBlobKzgCommitments(), "BlobKzgCommitments are required for EIP7594 blocks");
  ...
  @Override
  public BeaconBlockBodySchemaEip7594<?> getBeaconBlockBodySchema(final SpecVersion spec) {
    return (BeaconBlockBodySchemaEip7594<?>) spec.getSchemaDefinitions().getBeaconBlockBodySchema();
  }
